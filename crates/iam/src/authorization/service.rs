...
use crate::{data_scope::DataScopeFilter, users};
...
impl AuthorizationService {
    ...
    pub async fn snapshot(
        ...
        if let Some(cached) = cached {
            let snapshot: AuthorizationSnapshot = serde_json::from_str(&cached)?;
    ...
    pub async fn invalidate(&self) -> Result<(), AuthorizationError> {
        let Some(mut redis) = self.redis.clone() else {
            return Ok(());
        };
    ...
    async fn load_snapshot_from_db(
        ...
        if !enabled {
            return Err(AuthorizationError::UserDisabled);
        }
        ...
        let data_scope =
            crate::data_scope::resolve_user_data_scope(&self.pool, user_id, "authz").await?;
...
async fn load_catalog(pool: &PgPool) -> Result<AccessCatalog, AuthorizationError> {
