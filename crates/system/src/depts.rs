...
impl From<DeptError> for AppError {
    fn from(error: DeptError) -> Self {
        match error {
            DeptError::Database(error) => errors::depts::DEPT_DB_FAILED
                .into_error()
                .with_source(error),
            DeptError::InvalidParent => errors::depts::DEPT_INVALID_PARENT.into(),
        }
    }
}
...
pub async fn list(pool: &sqlx::PgPool) -> Result<Vec<Dept>, sqlx::Error> {
...
pub async fn tree(pool: &sqlx::PgPool) -> Result<Vec<DeptNode>, sqlx::Error> {
    let rows = list(pool).await?;
    Ok(build_dept_tree(rows))
}

pub async fn find(pool: &sqlx::PgPool, id: i64) -> Result<Option<Dept>, sqlx::Error> {
    sqlx::query_as::<_, Dept>(
        r#"
        select id, parent_id, name, code, sort, status
        from sys_depts
...
pub async fn create(pool: &sqlx::PgPool, payload: CreateDeptPayload) -> Result<(), sqlx::Error> {
    sqlx::query(
        r#"
