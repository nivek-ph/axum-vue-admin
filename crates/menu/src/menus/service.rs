...
pub struct MenuService {
    pool: PgPool,
    authorization: iam::authorization::AuthorizationService,
}

impl MenuService {
    pub fn new(pool: PgPool, authorization: iam::authorization::AuthorizationService) -> Self {
        Self {
            pool,
            authorization,
        }
    ...
    pub async fn current(&self, user_id: i64) -> Result<(Vec<MenuView>, Vec<String>), MenuError> {
        let snapshot = self.authorization.snapshot(user_id).await?;
