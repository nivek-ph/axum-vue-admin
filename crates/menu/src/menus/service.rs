...
use super::{MenuError, MenuMeta, MenuRecord, MenuRoleSelection, MenuView, SetMenuRolesRequest};

#[derive(Clone)]
pub struct MenuService {
    pool: PgPool,
    authorization: iam::authorization::service::AuthorizationService,
}
...
impl MenuService {
    ...
    pub async fn for_user(
        ...
        let is_super_admin = self
            .authorization
            .has_super_admin_role(user_id)
            .await
            .map_err(|error| match error {
                iam::authorization::AuthorizationError::Database(source) => {
                    MenuError::Database(source)
                }
            })?;
        get_menu_tree_for_user(&self.pool, user_id, is_super_admin).await
    }
    pub async fn ensure_default(&self) -> Result<(), MenuError> {
        Ok(ensure_default_menu(&self.pool).await?)
    }
    pub async fn list(&self) -> Result<Vec<MenuView>, MenuError> {
        get_menu_list(&self.pool).await
    }
    pub async fn tree(&self) -> Result<Vec<MenuView>, MenuError> {
        get_base_menu_tree(&self.pool).await
    }
    pub async fn create(&self, payload: MenuView) -> Result<(), MenuError> {
        add_base_menu(&self.pool, payload).await
    }
    pub async fn update(&self, payload: MenuView) -> Result<(), MenuError> {
        update_base_menu(&self.pool, payload).await
    }
    pub async fn delete(&self, id: i64) -> Result<(), MenuError> {
        delete_base_menu(&self.pool, id).await
    }
    pub async fn find(&self, id: i64) -> Result<MenuView, MenuError> {
        get_base_menu_by_id(&self.pool, id).await
    }
    pub async fn roles(&self, id: i64) -> Result<MenuRoleSelection, MenuError> {
        get_menu_roles(&self.pool, id).await
    }
    pub async fn set_roles(&self, payload: SetMenuRolesRequest) -> Result<(), MenuError> {
        set_menu_roles(&self.pool, payload).await
    }
}
...
pub fn default_menus() -> Vec<MenuView> {
    [
        ...
        (
            ...
            "departments",
            "Departments",
            "view/system/departments/index.vue",
            "building",
            vec![
                DefaultAction {
...
// ensure default menu
pub(crate) async fn ensure_default_menu(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
...
pub(crate) async fn get_menu_tree_for_user(
    pool: &sqlx::PgPool,
    user_id: i64,
    is_super_admin: bool,
) -> Result<Vec<MenuView>, MenuError> {
    ...
    if is_menu_super_admin_identity(is_super_admin) {
...
pub(crate) async fn get_menu_list(pool: &sqlx::PgPool) -> Result<Vec<MenuView>, MenuError> {
    let rows = load_menu_records(pool).await?;
...
pub(crate) async fn get_base_menu_tree(pool: &sqlx::PgPool) -> Result<Vec<MenuView>, MenuError> {
    get_menu_list(pool).await
}

pub(crate) async fn add_base_menu(pool: &sqlx::PgPool, payload: MenuView) -> Result<(), MenuError> {
    sqlx::query(
        r#"
        insert into sys_menus (
            parent_id, path, name, hidden, component, sort,
...
pub(crate) async fn update_base_menu(
    pool: &sqlx::PgPool,
    payload: MenuView,
) -> Result<(), MenuError> {
    sqlx::query(
...
pub(crate) async fn delete_base_menu(pool: &sqlx::PgPool, menu_id: i64) -> Result<(), MenuError> {
...
pub(crate) async fn get_base_menu_by_id(
    pool: &sqlx::PgPool,
    menu_id: i64,
) -> Result<MenuView, MenuError> {
...
pub(crate) async fn get_menu_roles(
    pool: &sqlx::PgPool,
    menu_id: i64,
...
pub(crate) async fn set_menu_roles(
    pool: &sqlx::PgPool,
