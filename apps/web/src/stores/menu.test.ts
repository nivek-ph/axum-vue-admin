...
describe('menu access state', () => {
  ...
  it('maps nested server menus and falls back to the first authorized route', () => {
    const items = buildMenuItems([{ name: 'system', children: [{ name: 'roles', path: 'roles' }] }, { name: 'files', path: 'files' }])
  ...
  it('allows every core route for super admin and resets access state', () => {
    useAuthStore.getState().setSession({ accessToken: 'a', refreshToken: 'r', userInfo: { id: 1, userName: 'admin', nickName: 'Admin', roles: [{ id: 1, code: 'super_admin', name: 'Super Admin' }] } })
