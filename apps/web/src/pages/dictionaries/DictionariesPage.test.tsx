...
describe('Dictionaries workflow', () => {
  ...
  it('creates a root detail within the selected dictionary and reloads its tree', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/menus/current') data = { code: 'OK', message: 'ok', data: { menus: [{ name: 'dictionaries', path: 'dictionaries' }], permissions: [] } }
      else if (config.url === '/dictionaries' && config.method === 'get') data = { code: 'OK', message: 'ok', data: [{ id: 7, name: 'Status', type: 'system_status', status: true, desc: '', parentId: null }] }
      else if (config.url === '/dictionaries/7/tree' && config.method === 'get') { treeReads += 1; data = { code: 'OK', message: 'ok', data: { list: [] } } }
      else if (config.url === '/dictionaries/7/tree' && config.method === 'post') { posted = { url: config.url, body: JSON.parse(String(config.data)) as Record<string, unknown> }; data = { code: 'OK', message: 'ok', data: {} } }
