...
describe('Roles workbench', () => {
  ...
  it('renders the complete role workbench in Chinese', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/menus/tree') data = { code: 'OK', message: 'ok', data: [{ id: 10, parentId: 0, path: '/system', name: 'system', sort: 1, menuType: 'directory', meta: { title: 'System' }, children: [{ id: 11, parentId: 10, path: '/users', name: 'users', sort: 1, menuType: 'page', meta: { title: 'Users' }, children: [{ id: 1101, parentId: 11, path: '', name: 'users:create', sort: 1, menuType: 'action', permission: 'system:user:create', meta: { title: 'Create user' }, children: [] }] }] }] }
      else if (config.url === '/roles/2/menus') data = { code: 'OK', message: 'ok', data: { menuIds: [] } }
      else if (config.url === '/roles') data = { code: 'OK', message: 'ok', data: { list: [{ id: 2, code: 'operator', name: 'Operator', status: 'enabled', sort: 1, data_scope: 'all', is_system: false }] } }
    ...
    expect(await screen.findByText('勾选页面访问会同时包含该页面下的按钮权限，避免页面可见但接口返回 403。')).toBeInTheDocument()
  ...
  it('saves an action permission with its ancestor closure but not sibling actions', async () => {
    ...
    const menuTree = [{
      id: 1, parentId: 0, path: 'system', name: 'system', sort: 1, meta: { title: 'System' }, children: [{
        id: 2, parentId: 1, path: 'users', name: 'users', sort: 1, menuType: 'page', meta: { title: 'Users' }, children: [
          { id: 20, parentId: 2, path: '', name: 'users:list', sort: 1, menuType: 'action', permission: 'system:user:list', meta: { title: 'List' }, children: [] },
          { id: 21, parentId: 2, path: '', name: 'users:create', sort: 2, menuType: 'action', permission: 'system:user:create', meta: { title: 'Create' }, children: [] },
        ],
      }],
    }]
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/roles/2/menus' && config.method === 'get') data = { code: 'OK', message: 'ok', data: { menuIds: [] } }
      else if (config.url === '/roles/2/menus' && config.method === 'put') { savedMenuIds = JSON.parse(String(config.data)).menuIds; data = { code: 'OK', message: 'saved' } }
      else if (config.url === '/roles') data = { code: 'OK', message: 'ok', data: { list: [{ id: 2, code: 'operator', name: 'Operator', status: 'enabled', sort: 1, data_scope: 'all', is_system: false }] } }
      else throw new Error(`Unexpected request: ${config.method} ${config.url}`)
      return { data, status: 200, statusText: 'OK', headers: {}, config }
    }) as AxiosAdapter
  ...
  it('hides the Members section without both role-user and user-list visibility permissions', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      let data: unknown
      if (config.url === '/users/me') data = { code: 'OK', message: 'ok', data: { userInfo: currentUser } }
      else if (config.url === '/menus/current') data = { code: 'OK', message: 'ok', data: { menus: [{ name: 'roles' }], permissions: ['system:role:list-users'] } }
