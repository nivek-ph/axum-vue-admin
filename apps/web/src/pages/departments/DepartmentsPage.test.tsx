...
describe('Departments workflow', () => {
  ...
  it('creates a child department through the scoped API and reloads the tree', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/menus/current') data = { code: 'OK', message: 'ok', data: { menus: [{ name: 'departments' }], permissions: [] } }
      else if (config.url === '/depts' && config.method === 'get') { reads += 1; data = { code: 'OK', message: 'ok', data: { list: [{ id: 3, parent_id: null, name: 'Operations', code: 'ops', sort: 1, status: 'enabled', children: [] }] } } }
      else if (config.url === '/depts' && config.method === 'post') { created = JSON.parse(String(config.data)); data = { code: 'OK', message: 'saved' } }
