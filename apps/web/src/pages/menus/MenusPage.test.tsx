...
describe('Access catalog', () => {
  ...
  afterEach(async () => { http.defaults.adapter = originalAdapter; await i18n.changeLanguage('en-US') })
  ...
  it('shows menu permissions and API bindings as a read-only catalog', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      const data = config.url === '/users/me'
        ...
        : config.url === '/menus/current'
          ...
          : { code: 'OK', message: 'ok', data: [{ id: 1, parentId: 0, path: 'organization', name: 'organization', sort: 1, menuType: 'directory', meta: { title: 'Organization' }, children: [{ id: 2, parentId: 1, path: 'users', name: 'users', sort: 1, menuType: 'page', permission: 'system:user:list', meta: { title: 'Users' }, apiBindings: [{ method: 'GET', pathPattern: '/api/users' }], children: [] }] }] }
