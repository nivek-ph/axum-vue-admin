...
describe('Files workflow', () => {
  ...
  beforeEach(() => { useAuthStore.getState().clearSession(); useMenuStore.getState().resetAccess() })
  ...
  it('uploads a selected file with the active category and refreshes the library', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/menus/current') data = { code: 'OK', message: 'ok', data: { menus: [{ name: 'files', path: 'files' }], permissions: [] } }
      else if (config.url === '/files' && config.method === 'get') { listReads += 1; data = { code: 'OK', message: 'ok', data: { list: [], total: 0, page: 1, pageSize: 10 } } }
      else if (config.url === '/files/upload') { uploadCategory = config.params?.category; uploadedName = (config.data as FormData).get('file') instanceof File ? ((config.data as FormData).get('file') as File).name : ''; data = { code: 'OK', message: 'ok', data: {} } }
    ...
    await user.upload(container.querySelector('input[type="file"]') as HTMLInputElement, new File(['report'], 'report.txt', { type: 'text/plain' }))
