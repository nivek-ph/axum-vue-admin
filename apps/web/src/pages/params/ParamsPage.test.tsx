...
describe('Params workflow', () => {
  ...
  it('edits and deletes a param with application confirmation and list refresh', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      ...
      else if (config.url === '/menus/current') data = { code: 'OK', message: 'ok', data: { menus: [{ name: 'params' }], permissions: [] } }
      else if (config.url === '/params' && config.method === 'get') data = { code: 'OK', message: 'ok', data: { list: [{ id: 8, name: 'Theme', key: 'ui.theme', value: 'light', desc: '' }], total: 1, page: 1, pageSize: 10 } }
      else if (config.url === '/params/8' && config.method === 'put') { updated = JSON.parse(String(config.data)); data = { code: 'OK', message: 'saved' } }
      else if (config.url === '/params/8' && config.method === 'delete') { deleted = true; data = { code: 'OK', message: 'deleted' } }
