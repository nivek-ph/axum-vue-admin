...
function rejectEnvelope(config: InternalAxiosRequestConfig, code: string, status = 401) {
  return Promise.reject(new AxiosError('request failed', String(status), config, undefined, {
  ...
  it('shares one token refresh across concurrent expired requests', async () => {
    ...
    http.defaults.adapter = (async (config) => {
      if (config.url === '/auth/refresh') {
        ...
        return response(config, { code: 'OK', message: 'ok', data: { accessToken: 'access-new', refreshToken: 'refresh-new' } })
      }
