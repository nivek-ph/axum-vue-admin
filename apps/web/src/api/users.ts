...
export interface UserListResult { list: UserRecord[]; total: number; page: number; pageSize: number }
...
export async function fetchUsers(page = 1, pageSize = 10) {
  const response = await http.get<never, ApiEnvelope<UserListResult>>('/users', { ...withAuthHeaders(), params: { page, pageSize } })
  return {
...
export function createUser(form: CreateUserForm) {
  return http.post<never, ApiEnvelope>('/users', {
