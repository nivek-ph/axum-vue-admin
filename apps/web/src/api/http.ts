...
const terminalCodes = new Set(['ACCESS_TOKEN_EXPIRED', 'LOGIN_REQUIRED', 'TOKEN_INVALID', 'SESSION_INVALID', 'REFRESH_TOKEN_INVALID', 'USER_DISABLED'])
...
function rejectedError(error: AxiosError) {
  ...
  if (isApiEnvelope(body)) return new ApiHttpError(body.message || 'Request failed', { status: error.response?.status, body, cause: error })
  return error
}
