...
use super::dto::{
    FileDeletePayload, FileEditPayload, FileListQuery, FileResponse, ImportUrlPayload,
};
...
pub async fn get_file_list(
    ...
    Json(payload): Json<FileListQuery>,
...
pub async fn delete_file(
    State(state): State<AppState>,
    Json(payload): Json<FileDeletePayload>,
) -> AppResult<Json<ApiResponse<Value>>> {
    state.files.delete(payload.id).await.map_err(map_error)?;
...
pub async fn edit_file_name(
    State(state): State<AppState>,
    Json(payload): Json<FileEditPayload>,
) -> AppResult<Json<ApiResponse<Value>>> {
    state
        .files
...
pub async fn upload_file(
        ...
        .map_err(errors::multipart_field_error)?
    {
        ...
        uploaded = Some(FileResponse::from(
            state
                .files
                .upload(&file_name, query.class_id.unwrap_or(0), bytes.as_ref())
                .await
                .map_err(map_error)?,
        ));
    }
